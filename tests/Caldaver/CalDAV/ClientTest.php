...
use Caldaver\Http\Client as HttpClient;
use Caldaver\XML\Toolkit;
use Caldaver\XML\Generator;
use Caldaver\XML\Parser;
...
use Caldaver\Event\Parser as EventParser;
...
{
    /** @var \Caldaver\Http\Client */
    protected $http_client;

    /** @var \Caldaver\XML\Generator */
    protected $xml_generator;

    /** @var array */
    protected $history;

    /** @var \Caldaver\XML\Parser */
    protected $xml_parser;

    /** @var Caldaver\Event\Parser */
    ...
    public function setUp(): void
    {
        ...
        $this->event_parser = $this->createMock(\Caldaver\Event\Parser::class);
        $this->history = [];
    }
    ...
    {
        $this->expectException(\Caldaver\Exception\NotFound::class);
        $body = <<<BODY
...
BODY;
        ...
        $this->assertInstanceOf(
          '\Caldaver\CalDAV\Resource\Calendar',
          $calendar
        );
    }
...
BODY;
      ...
      $this->event_parser
        ...
        ->method('parse')
        ->willReturn(
          $this->createMock(\Caldaver\Event::class),
          $this->createMock(\Caldaver\Event::class)
        );
      ...
      $this->assertInstanceOf('\Caldaver\Event', $objects[0]->getEvent());
      ...
      $this->assertInstanceOf('\Caldaver\Event', $objects[1]->getEvent());
...
BODY;
        ...
        $this->assertInstanceOf('\Caldaver\Event', $object->getEvent());
    ...
    public function testPutObjectNew()
    {
        ...
        // the validity check
        $event = $this->createMock(\Caldaver\Event::class);
        $event
          ->expects($this->exactly(2))
          ->method('render')
    ...
    public function testACL()
    {
        ...
        $acl = $this->createMock(\Caldaver\CalDAV\Share\ACL::class);
