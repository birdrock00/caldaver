...
fn render_calendar(state: &AppState, session: &Session) -> String {
    let content = format!(
        r#"{navbar}<main class="container-fluid calendar-shell"><div id="wrapper" class="calendar-layout"><div id="sidebar">{sidebar}</div><div id="content"><div id="calendar_view"></div></div></div></main><nav id="mobile_bottom_bar" class="mobile-bottom-bar" aria-label="Calendar quick actions" hidden><button type="button" class="mobile-bottom-btn" data-mobile-action="prev" aria-label="Previous" title="Previous"><i class="fa fa-chevron-left" aria-hidden="true"></i><span class="mobile-bottom-btn-label"><i class="fa fa-chevron-left" aria-hidden="true"></i></span></button><button type="button" class="mobile-bottom-btn" data-mobile-action="today" aria-label="Today" title="Today"><i class="fa fa-dot-circle-o" aria-hidden="true"></i><span class="mobile-bottom-btn-label">Today</span></button><button type="button" class="mobile-bottom-btn" data-mobile-action="view" aria-label="Change view" title="Change view"><i class="fa fa-th-list" aria-hidden="true"></i><span class="mobile-bottom-btn-label">View</span></button><button type="button" class="mobile-bottom-btn" data-mobile-action="refresh" aria-label="Refresh" title="Refresh"><i class="fa fa-refresh" aria-hidden="true"></i><span class="mobile-bottom-btn-label">Refresh</span></button><button type="button" class="mobile-bottom-btn" data-mobile-action="next" aria-label="Next" title="Next"><i class="fa fa-chevron-right" aria-hidden="true"></i><span class="mobile-bottom-btn-label"><i class="fa fa-chevron-right" aria-hidden="true"></i></span></button></nav><button type="button" id="mobile_fab_add" class="mobile-fab" aria-label="Create event" title="Create event" hidden><i class="fa fa-plus" aria-hidden="true"></i></button><div id="mobile_ptr" class="mobile-ptr" aria-hidden="true" hidden><i class="fa fa-refresh" aria-hidden="true"></i></div><div id="etar_calendar_view_popup" class="etar-calendar-view-popup" role="menu" aria-label="Calendar view" hidden><button type="button" class="etar-calendar-view" data-etar-view="agendaDay" role="menuitemradio" aria-checked="false"><i class="fa fa-calendar-o" aria-hidden="true"></i><span>Day</span></button><button type="button" class="etar-calendar-view" data-etar-view="agendaWeek" role="menuitemradio" aria-checked="false"><i class="fa fa-columns" aria-hidden="true"></i><span>Week</span></button><button type="button" class="etar-calendar-view" data-etar-view="month" role="menuitemradio" aria-checked="false"><i class="fa fa-th" aria-hidden="true"></i><span>Month</span></button><button type="button" class="etar-calendar-view" data-etar-view="customizable_list" role="menuitemradio" aria-checked="false"><i class="fa fa-list-ul" aria-hidden="true"></i><span>Agenda</span></button></div><button id="shortcut_add_event" class="create-event-button" aria-label="Create event" title="Create event"><i class="fa fa-plus" aria-hidden="true"></i><span class="visually-hidden">Create event</span></button>"#,
...
fn layout(state: &AppState, body_class: &str, content: &str, bottom: &str) -> String {
    format!(
        r#"<!DOCTYPE html><html lang="en"><head><title>{title}</title><link rel="shortcut icon" href="/img/favicon.ico"><meta http-equiv="content-type" content="text/html; charset=utf-8"><meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover"><link href="/dist/css/caldaver.css" rel="stylesheet" type="text/css"><link href="/dist/css/caldaver.print.css" rel="stylesheet" type="text/css" media="print"></head><body class="{body_class}">{content}{bottom}</body></html>"#,
...
fn sidebrand() -> &'static str {
    "" // [B1] Sidebar Caldaver logo removed; kept as a no-op for callers.
}

fn calendar_sidebar() -> String {
    format!(
        r#"{sidebrand}{appnav}<div class="block calendar_list panel panel-default shared_calendars calendar-sidebar-section" id="shared_calendar_list"><div class="panel-heading"><h3 class="panel-title">Shared calendars</h3></div><div class="panel-body"><ul class="fa-ul"><li class="calendar-list-loading">Loading shared calendars...</li></ul><div class="buttons"><button type="button" id="shared_calendar_add" class="pseudobutton" title="Create" aria-label="Create"><i class="fa fa-plus" aria-hidden="true"></i></button><button type="button" id="toggle_all_shared_calendars" class="pseudobutton hide_all" title="Show/hide all" aria-label="Show/hide all"><i class="fa fa-eye-slash fa-lg" aria-hidden="true"></i></button></div></div></div>"#,
        sidebrand = sidebrand(),
...
fn contact_dialog(csrf: &str) -> String {
    format!(
        r#"<div id="contact_dialog" class="contact-dialog" role="dialog" aria-modal="true" aria-labelledby="contact_dialog_title" hidden><form id="contact_form" action="/cards/save" method="post"><input type="hidden" name="_token" value="{csrf}"><input type="hidden" name="url" value=""><input type="hidden" name="etag" value=""><input type="hidden" name="uid" value=""><div class="contact-dialog-panel"><div class="contact-dialog-header"><h2 id="contact_dialog_title">Create contact</h2><button type="button" id="contact_cancel_icon" aria-label="Cancel"><i class="fa fa-times"></i></button></div><label for="contact_full_name"><span>Name</span><input id="contact_full_name" required name="full_name" type="text" autocomplete="name" autocapitalize="words" enterkeyhint="next"></label><label for="contact_email"><span>Email</span><input id="contact_email" name="email" type="email" autocomplete="email" inputmode="email" autocapitalize="none" autocorrect="off" spellcheck="false" enterkeyhint="next"></label><label for="contact_phone"><span>Phone number</span><input id="contact_phone" name="phone" type="tel" autocomplete="tel" inputmode="tel" enterkeyhint="next"></label><label for="contact_organization"><span>Company</span><input id="contact_organization" name="organization" type="text" autocomplete="organization" autocapitalize="words" enterkeyhint="next"></label><label for="contact_job_title"><span>Job title</span><input id="contact_job_title" name="job_title" type="text" autocomplete="organization-title" autocapitalize="words" enterkeyhint="done"></label><div class="contact-dialog-footer"><button type="button" id="contact_cancel" class="btn btn-default">Cancel</button><button type="submit" class="btn btn-primary">Save</button></div></div></form></div>"#
    )
}

fn account_dialog(csrf: &str) -> String {
    format!(
        r#"<div id="mail_account_dialog" class="contact-dialog account-dialog" role="dialog" aria-modal="true" aria-labelledby="mail_account_dialog_title" hidden><form id="mail_account_form" action="/accounts/save" method="post"><input type="hidden" name="_token" value="{csrf}"><input type="hidden" name="id" value=""><div class="contact-dialog-panel"><div class="contact-dialog-header"><h2 id="mail_account_dialog_title">Add account</h2><button type="button" id="mail_account_cancel_icon" aria-label="Cancel"><i class="fa fa-times"></i></button></div><fieldset class="account-type-chooser" aria-label="Account type"><label><input type="radio" name="account_type" value="calendar" checked> <span>Calendar</span></label><label><input type="radio" name="account_type" value="carddav"> <span>Contacts</span></label><label><input type="radio" name="account_type" value="email"> <span>Email</span></label></fieldset><div class="account-common-fields"><label for="account_label"><span>Account name</span><input id="account_label" required name="label" type="text" autocomplete="organization" autocapitalize="words" enterkeyhint="next"></label><label data-account-field="dav" for="account_server_url"><span>DAV server URL</span><input id="account_server_url" name="server_url" type="url" inputmode="url" autocomplete="url" autocapitalize="none" autocorrect="off" spellcheck="false" enterkeyhint="next" placeholder="https://dav.example.com/"></label><label data-account-field="dav" for="account_auth_method"><span>Auth method</span><select id="account_auth_method" name="auth_method"><option value="basic">Basic password</option><option value="bearer">Bearer token</option><option value="none">None</option></select></label><label data-account-field="email" for="account_email_address"><span>Email address</span><input id="account_email_address" name="email_address" type="email" autocomplete="email" inputmode="email" autocapitalize="none" autocorrect="off" spellcheck="false" enterkeyhint="next"></label><label data-account-field="email" for="account_imap_host"><span>IMAP host</span><input id="account_imap_host" name="imap_host" type="text" autocomplete="off" inputmode="url" autocapitalize="none" autocorrect="off" spellcheck="false" enterkeyhint="next"></label><label data-account-field="email" for="account_imap_port"><span>IMAP port</span><input id="account_imap_port" name="imap_port" type="number" inputmode="numeric" min="1" max="65535" value="993" enterkeyhint="next"></label><label data-account-field="email" for="account_encryption"><span>Encryption</span><select id="account_encryption" name="encryption"><option value="ssl">SSL</option><option value="tls">STARTTLS</option><option value="none">None</option></select></label><label for="account_username"><span>User name</span><input id="account_username" required name="username" type="text" autocomplete="username" autocapitalize="none" autocorrect="off" spellcheck="false" enterkeyhint="next"></label><label for="account_password"><span>Password or token</span><input id="account_password" name="password" type="password" autocomplete="current-password" autocapitalize="none" autocorrect="off" spellcheck="false" enterkeyhint="done"><small class="account-credential-help">Saved credentials are never shown. Leave this blank when editing to keep the current credential.</small></label><label data-account-field="dav" for="account_home_set"><span>Home set</span><input id="account_home_set" name="home_set" type="text" autocomplete="off" autocapitalize="none" autocorrect="off" spellcheck="false" enterkeyhint="next" placeholder="/calendars/user/"></label><label data-account-field="email" for="account_refresh_interval"><span>Auto refresh minutes</span><input id="account_refresh_interval" name="refresh_interval_minutes" type="number" inputmode="numeric" min="1" max="1440" value="1" enterkeyhint="done"></label></div><div id="mail_account_error" class="mail-error" aria-live="assertive" hidden></div><div class="contact-dialog-footer"><button type="button" id="mail_account_cancel" class="btn btn-default">Cancel</button><button type="submit" class="btn btn-primary">Save</button></div></div></form></div>"#
    )
}
